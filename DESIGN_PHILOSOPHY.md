...
```text
src/prompts/
├── systemRules.ts            <-- 共通ルール（日本語出力強制・戦略アプローチ化・Deliverable Action化・subtasks同時生成・操作マニュアル化禁止）
├── strategyPrompt.ts         <-- Goalから時間予算(appetiteHours)・実施時期(timeframe)付きStrategy(戦略アプローチ)およびPhase 1 Deliverable Action+subtasksを提案するJSONプロンプト
├── taskBreakdownPrompt.ts    <-- 合意Strategyから1〜3時間成果物(Deliverable) Actionと内部15〜30分ステップ(subtasks)をローリングウェーブ分解するJSONプロンプト
...
1. **`Strategy` 提案フェーズ (`buildStrategyPrompt` / `AIService.generateStrategyWithContext`)**:
   - 各提案作戦 (`proposedStrategies`) は単なる工程名ではなく戦い方・アプローチとして提案し、投資上限時間枠 `appetiteHours` (例: 40) と相対時期 `timeframe` (例: "今月") を必須保持。
...
2. **`Action` ローリングウェーブ分解フェーズ (`buildTaskBreakdownPrompt` / `AIService.breakdownTaskWithContext`)**:
   - 直近 Strategy を 1〜3時間の明確な成果物 (Deliverable: 仕様書、コード、検証結果等) が得られる Action ノードへ分解。
   - 単一PC操作（「ブラウザを開く」「メモする」等）のマニュアル化を禁止。
   - 各 Action ノード内部に 15〜30分単位で実行可能な具体手順・章立て (`subtasks`) 配列を同時生成・Frontmatter (`subtasks`) へ保存。
