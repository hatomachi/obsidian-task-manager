...
- [ ] **Phase 3: 親子リンク（`parent_task`）とサブタスクの昇格（Promote）【🚧 次の対象】**
  ...
  - **完了条件 (DoD)**:
    - [ ] エディタで `- [ ] サブタスク` の行にカーソルを置き、コマンドを実行すると新ワークフォルダが生成されること。
    - [ ] 子 `index.md` の Frontmatter に親ノートへの Wiki リンク `parent_task: "[[.../index]]"` が記録されること。
    - [ ] 親ノートの元のインラインタスク行が、子ワークフォルダへの Wiki リンクに自動書き換えされること。
    - [ ] `npm run build` が正常に通り、動作検証が完了すること。
